...
def parse_approvals(data: list) -> list[Approval]:
    """Parse active approval data from Keel API response.

    Keel's approvals endpoint can include archived records. Those records are
    historical and are not approvable; treating them as pending causes repeated
    404s when the bot tries to approve them.
    """
    approvals = []
    archived_count = 0
    for item in data:
    ...
    if archived_count:
        print(f"[{datetime.now().isoformat()}] Ignored {archived_count} archived approval(s)")
    return approvals
...
def format_release_notes_line(target, label: str = "Release Notes") -> str:
    """Format the release-notes row for an approval or raw identifier."""
    if isinstance(target, Approval):
        release_notes_key = get_release_notes_key(target)
    else:
        identifier = str(target)
        release_notes_key = get_resource_name_from_identifier(identifier) or identifier
    return f"📝 {label} ({release_notes_key}): {get_release_notes_url(target)}"
...
def format_approval_message(
    ...
    lines.append(format_release_notes_line(approval, "Release Notes URL"))
...
def normalize_release_notes_key(key: str) -> str:
    """Return the canonical key used for release-notes URL lookups."""
    return str(key or "").strip().lower()


DEFAULT_RELEASE_NOTES_URLS = {
    "actualbudget/actual-server": "https://www.actualbudget.com/docs/releases/",
    "codewithcj/sparkyfitness": "https://github.com/CodeWithCJ/SparkyFitness/releases",
    "codewithcj/sparkyfitness_server": "https://github.com/CodeWithCJ/SparkyFitness/releases",
    "freikin/dawarich": "https://github.com/Freika/dawarich/releases",
    "library/mariadb": "https://mariadb.org/mariadb/all-releases/",
}


def infer_release_notes_url(target) -> str:
    """Infer a release-notes URL from Keel repository metadata."""
    if not isinstance(target, Approval):
        return "N/A"

    repository_name = normalize_repository_name(target.repository_name)
    if not repository_name:
        return "N/A"

    repository_key = normalize_release_notes_key(repository_name)
    for prefix in ("index.docker.io/", "docker.io/"):
        if repository_key.startswith(prefix):
            repository_key = repository_key[len(prefix):]
            break

    if repository_key in DEFAULT_RELEASE_NOTES_URLS:
        return DEFAULT_RELEASE_NOTES_URLS[repository_key]

    if repository_key.startswith("ghcr.io/"):
        parts = repository_key[len("ghcr.io/"):].split("/")
        if len(parts) >= 2:
            if parts[0] == "imagegenius":
                return f"https://github.com/imagegenius/docker-{parts[1]}/releases"
            return f"https://github.com/{parts[0]}/{parts[1]}/releases"

    if repository_key.startswith("lscr.io/linuxserver/"):
        image_name = repository_key.rsplit("/", 1)[-1]
        return f"https://github.com/linuxserver/docker-{image_name}/releases"

    if repository_key.startswith("linuxserver/"):
        image_name = repository_key.rsplit("/", 1)[-1]
        return f"https://github.com/linuxserver/docker-{image_name}/releases"

    if repository_key.startswith("imagegenius/"):
        image_name = repository_key.rsplit("/", 1)[-1]
        return f"https://github.com/imagegenius/docker-{image_name}/releases"

    if repository_key.startswith("library/"):
        image_name = repository_key.rsplit("/", 1)[-1]
        return f"https://hub.docker.com/_/{image_name}/tags"

    if "/" in repository_key:
        namespace, image_name = repository_key.split("/", 1)
        return f"https://hub.docker.com/r/{namespace}/{image_name}/tags"
...
def format_approvals_list(
    ...
    for i, approval in enumerate(approvals, 1):
        ...
        lines.append(f"   {format_release_notes_line(approval)}")
...
def format_matrix_html(message: str) -> str:
    """Render Markdown links and bare URLs to Matrix formatted HTML."""

    bare_url_pattern = re.compile(r'https?://[^\s<>"\']+')
    markdown_link_pattern = re.compile(r'\[([^\]]+)\]\((https?://[^\s)]+)\)')

    def escape_text_with_bare_urls(text: str) -> str:
        output = []
        last_index = 0
        for match in bare_url_pattern.finditer(text):
            ...
            output.append(f'<a href="{href}">{html.escape(raw_url)}</a>')
    ...
    for match in markdown_link_pattern.finditer(message):
        output.append(escape_text_with_bare_urls(message[last_index:match.start()]))
        label = html.escape(match.group(1))
        href = html.escape(match.group(2), quote=True)
        output.append(f'<a href="{href}">{label}</a>')
        last_index = match.end()
    output.append(escape_text_with_bare_urls(message[last_index:]))
...
async def send_matrix_message(
    ...
    formatted_body = format_matrix_html(message)
...
class ApprovalMemory:
    ...
    def _database_config_from_env(self) -> Optional[dict]:
        ...
        return {
            ...
            'max_size': 4,
            'command_timeout': 10,
            'server_settings': {'application_name': 'keel-matrix-bot'},
        }

    def _ensure_loaded(self):
        """Initialize required PostgreSQL state storage and load persisted state."""
        if self._loaded:
            return

        if asyncpg is None:
            raise RuntimeError("PostgreSQL state storage requires asyncpg to be installed")

        self._db_config = self._database_config_from_env()
        if not self._db_config:
            raise RuntimeError(
                "PostgreSQL state storage is required. Set POSTGRES_HOST, "
                "POSTGRES_USER, and POSTGRES_PASSWORD."
            )

        try:
            self._run_db_with_pool(self._ensure_schema)
            print(
            ...
            raise RuntimeError(f"PostgreSQL state storage is unavailable: {e}") from e

    async def _with_db_pool(self, operation):
        pool = await asyncpg.create_pool(**self._db_config)
        try:
    ...
    async def _ensure_schema(self, pool):
        ...
        await pool.execute("""
            CREATE TABLE IF NOT EXISTS keel_bot_state (
    ...
    async def _load_all_from_db(self, pool) -> dict:
        """Load all state rows from PostgreSQL."""
    ...
    async def _save_all_to_db(self, pool, data: dict):
        """Upsert all state rows into PostgreSQL."""
    ...
    def _load_state(self):
        ...
        db_data = self._run_db_with_pool(self._load_all_from_db)

        if db_data:
            self._apply_state(db_data)
            print(
                f"[DB] Loaded state from PostgreSQL: "
                ...
                f"{len(self.auto_approve_failures)} auto-approve failure(s)"
            )
            return

        self._reset_state()
        print("[DB] No existing PostgreSQL state rows found; starting with empty state")
    ...
    def get_approval_timestamps(self) -> dict:
        """Return approval first-seen timestamps."""
        self._ensure_loaded()
        with self._lock:
            return dict(self.approval_timestamps)

    def get_release_notes_urls(self, require_loaded: bool = True) -> dict:
        """Return configured release-notes URLs."""
        if require_loaded:
            self._ensure_loaded()
        elif not self._loaded:
            return {}
        with self._lock:
            return dict(self.release_notes_urls)
    ...
    def set_release_notes_url(self, identifier: str, url: str) -> str:
        """Set a release-notes URL using a case-insensitive canonical key."""
        self._ensure_loaded()
        normalized_identifier = normalize_release_notes_key(identifier)
        if not normalized_identifier:
            return ""

        with self._lock:
            duplicate_keys = [
                key for key in self.release_notes_urls
                if normalize_release_notes_key(key) == normalized_identifier
            ]
            for key in duplicate_keys:
                self.release_notes_urls.pop(key, None)

            self.release_notes_urls[normalized_identifier] = url.strip()
            self._save_state()
...
def has_auto_approve_failed(approval: Approval) -> bool:
    """Return whether auto-approval already failed for this approval ID."""
    return approval_memory.has_auto_approve_failed(approval)

def mark_auto_approve_failed(approval: Approval, reason: str):
    """Remember a failed auto-approve attempt for this approval ID."""
    approval_memory.mark_auto_approve_failed(approval, reason)

def set_release_notes_url(identifier: str, url: str):
    """Set the release notes URL for an approval identifier."""
    stored_identifier = approval_memory.set_release_notes_url(identifier, url)
    print(
        f"[{datetime.now().isoformat()}] Set release notes URL for "
        f"'{stored_identifier or identifier}': {url.strip()}"
    )
...
def get_approval_timestamps() -> dict:
    """Return approval first-seen timestamps."""
    return approval_memory.get_approval_timestamps()
...
def get_release_notes_url(target) -> str:
    ...
    release_notes_urls = approval_memory.get_release_notes_urls(require_loaded=False)
    configured_urls = {
        normalize_release_notes_key(key): url
        for key, url in release_notes_urls.items()
        if normalize_release_notes_key(key)
    }

    # Try exact match first against every derived key, case-insensitively.
    for key in lookup_keys:
        normalized_key = normalize_release_notes_key(key)
        if normalized_key in configured_urls:
            return configured_urls[normalized_key]

    # Try partial matching, preferring the most specific overlap. This prevents
    # an older broad or differently-cased key from winning just because it was
    # inserted earlier in persisted state.
    best_match = None
    lookup_keys_lower = [normalize_release_notes_key(key) for key in lookup_keys]
    for key, url in release_notes_urls.items():
        key_lower = normalize_release_notes_key(key)
        for lookup_key in lookup_keys_lower:
            if not key_lower or not lookup_key:
                continue
            if key_lower in lookup_key or lookup_key in key_lower:
                score = min(len(key_lower), len(lookup_key))
                if best_match is None or score > best_match[0]:
                    best_match = (score, url)

    if best_match is not None:
        return best_match[1]

    inferred_url = infer_release_notes_url(target)
    if inferred_url != "N/A":
        return inferred_url
...
class KeelMatrixBot:
    ...
    async def process_http_approval_action_request(self, identifier: str, action: str) -> tuple[int, str]:
        ...
        if raw_response.get("error"):
            return 502, "Unable to verify whether this approval is still pending."

        if not any(approval.identifier == clean_identifier for approval in approvals):
            print(
                f"[{datetime.now().isoformat()}] Ignoring stale HTTP approval click for "
                f"{clean_identifier}; no active pending approval exists"
            )
            return (
                200,
                f"No pending approval remains for '{clean_identifier}'. "
                f"It was already processed or expired; no {get_action_noun(normalized_action)} was submitted."
            )

        # Reuse existing approval flow so behavior stays consistent with Matrix commands.
        result = await self.handle_approve_reject(self.room_id, clean_identifier, normalized_action)
        if isinstance(result, tuple) and len(result) == 2:
            return result
        return 202, f"{get_action_label(normalized_action)} request submitted for '{clean_identifier}'."
    ...
    async def auto_approve_matching_approvals(self, room_id: str, approvals: list[Approval]) -> set[str]:
        ...
        for approval in approvals:
            ...
            if has_auto_approve_failed(approval):
                print(
                    f"[{datetime.now().isoformat()}] Skipping auto-approve retry for "
                    f"{approval.identifier} ({approval.id}); prior failure already recorded"
                )
                continue
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                try:
                    ...
                    if response.status_code in (200, 201):
                        auto_approved_identifiers.add(approval.identifier)
                        self.recently_processed_approvals.add(auto_dedup_key)
                        mark_as_notified([approval])
                        response_msg = (
                            f"✅ **Auto-approve submitted**\n"
                            f"\n"
                            f"Identifier: `{approval.identifier}`\n"
                            f"Approval ID: `{approval.id}`\n"
                            f"Update: `{approval.current_version}` → `{approval.new_version}`\n"
                            f"\n"
                            f"Keel will process the approved update. The approval was not deleted.\n"
                            f"\n"
                            f"{SEPARATOR}"
                        )
                    else:
                        failure_reason = f"approve failed with status {response.status_code}: {response.text}"
                        mark_auto_approve_failed(approval, failure_reason)
                        response_msg = (
                            ...
                            f"\n"
                            f"This approval will not be auto-retried unless Keel creates a new approval ID.\n"
                            f"\n"
                            f"{SEPARATOR}"
                        )
                except httpx.RequestError as e:
                    mark_auto_approve_failed(approval, f"request error: {str(e)}")
                    response_msg = (
    ...
    async def handle_approve_reject(self, room_id: str, identifier: str, action: str):
        ...
        try:
            ...
            if not raw_response.get("error"):
                for approval in approvals:
                    if approval.identifier == identifier:
                        pre_action_approval = approval
                        break
        except Exception as e:
            print(
                f"[{datetime.now().isoformat()}] Unable to capture approval metadata "
                f"before {action}: {type(e).__name__}: {e}"
            )
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            try:
                ...
                if response.status_code in (200, 201):
                    result_status = 200
                    # Success: do not delete approvals after approving them.
                    # Keel needs the approved approval record to complete the rollout.
                    approved_approval = pre_action_approval
                    ...
                    release_notes_line = format_release_notes_line(approved_approval or identifier)

                    if action == "approve":
                        if approved_approval:
                            mark_as_notified([approved_approval])
                        response_msg = (
                            f"✅ **Approve submitted!**\n"
                            f"\n"
                            f"Identifier: `{identifier}`\n"
                            f"Approval ID: `{approved_approval.id if approved_approval else 'unknown'}`\n"
                            f"{release_notes_line}\n"
                            f"\n"
                            f"Keel will process the approved update. The approval was not deleted.\n"
                            f"\n"
                            f"{SEPARATOR}"
                        )
                    elif approved_approval:
                        delete_payload = {
                            "id": approved_approval.id,
                            "identifier": identifier,
                            "action": "delete",
                            "voter": self.user_id
                        }
                        ...
                        if delete_response.status_code in (200, 201):
                            ...
                            response_msg = (
                                ...
                                f"\n"
                                f"Identifier: `{identifier}`\n"
                                f"Approval ID: `{approved_approval.id}`\n"
                                f"{release_notes_line}\n"
                                f"\n"
                                f"{SEPARATOR}"
                            )
                    ...
                    result_status = 200 if stale_deleted else 404
                    stale_delete_msg = (
        ...
        return result_status, response_msg
...
async def main():
    ...
    parser.add_argument(
        "--keel-url",
        default=os.environ.get("KEEL_URL", ""),
    ...
    print("  State Storage: PostgreSQL")
    ...
    try:
        approval_memory._ensure_loaded()
